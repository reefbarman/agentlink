...
export const executeCommandSchema = {
  ...
  sandbox_permissions: z
    ...
    .optional()
    .describe(
      'Execution authority intent. Omit or use "use_default" for the normal sandbox route with loopback client access but public/LAN egress and listener binding blocked. Use "with_additional_permissions" with additional_permissions for a narrow sandbox capability such as local listener binding. Recognized default-sandbox HOME/listener failures may return retry_guidance code "sandbox_missing_capabilities" with the exact narrow retry parameters; changed sandbox preparation security may return "sandbox_preparation_changed" with the changed fields; predictable Git metadata writers may return "protected_git_metadata" guidance before launch. When every command segment matches an Allow command rule, that rule authorizes the requested route and capabilities; validation and explicit Forbidden rules still apply. "require_managed_network" is the compatibility intent for one exact sandboxed command that needs reviewed public network access; Git-over-SSH and recognized gh TLS trust failures may return non-automatic retry_guidance codes "managed_network_ssh_git_transport" or "managed_network_tls_trust". Never replace trust repair with disabled TLS verification. Use "require_escalated" only when execution must occur outside the sandbox. A recognized sandbox denial after process launch requires a one-shot human approval before native replay, even when a matching command Allow rule exists. Every non-default intent requires a non-empty reason; an uncovered command requires approval.',
    ),
