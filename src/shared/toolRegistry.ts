...
export const TOOL_REGISTRY: Record<string, ToolMeta> = {
  ...
  execute_command: {
    ...
    description:
      "Run a command in AgentLink's managed terminal. Sequential calls automatically reuse an idle compatible terminal; overlapping implicit calls allocate separate terminals when needed. Use `terminal_name` when a terminal should have a stable purpose-based label such as 'Dev server', 'Unit tests', or 'Build', or `terminal_id` to target a specific existing terminal. Use `background` for long-running processes and `timeout` for quick commands. Submit the simplest review-friendly command: AgentLink already disables interactive pagers consistently, so do not add `GIT_PAGER=cat`, `PAGER=cat`, or routine `--no-pager` workarounds. If the response includes `output_file`, read that file instead of re-running the command. Piped `grep`/`head`/`tail` patterns are rejected; use `output_grep`, `output_head`, or `output_tail` instead. Commands use the normal policy-selected route with sandbox network blocked by default. Set `sandbox_permissions` to `require_managed_network` with a non-empty `reason` for one exact sandboxed command that needs public network access; local and private destinations remain blocked and Approve for Me sends the request to Guardian. Set it to `require_escalated` only when execution must occur outside the sandbox; this also requires a non-empty reason, command approval rules, and approval when no rule matches.",
  },
  get_terminal_output: {
