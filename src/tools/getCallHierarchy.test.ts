...
import { handleGetCallHierarchy } from "./getCallHierarchy.js";
...
describe("handleGetCallHierarchy", () => {
  it("returns an explicit unavailable result when no hierarchy provider is supplied", async () => {
    const result = await handleGetCallHierarchy(params, "session-1");
  ...
  it("delegates call hierarchy params to the provider", async () => {
    const getCallHierarchy = vi.fn(async () => ({
      content: [{ type: "text" as const, text: JSON.stringify({ ok: true }) }],
    }));

    const result = await handleGetCallHierarchy(params, "session-1", {
      hierarchyProvider: { getCallHierarchy, getTypeHierarchy: vi.fn() },
    });

    expect(textPayload(result)).toEqual({ ok: true });
    expect(getCallHierarchy).toHaveBeenCalledWith({
      path: "src/file.ts",
      line: 2,
      column: 3,
      direction: "both",
  ...
  it("wraps provider errors in the legacy JSON error shape", async () => {
    const getCallHierarchy = vi.fn(async () => {
      throw new Error("bad call hierarchy lookup");
    });
    ...
    expect(textPayload(result)).toEqual({
      error: "bad call hierarchy lookup",
      path: "src/file.ts",
    });
  });
