...
## Shared native credential storage

`createKeychainSecretStorage(...)` exposes one explicitly named OS credential entry through the storage contract used by host-owned authentication. Reads, writes, and deletion remain in the native host process; renderers and browser clients receive only display-safe account metadata or short-lived capabilities chosen by the host. Mutations use the adapter's same-machine directory lock so several AgentLink processes re-read and update one credential state without ordinary stale overwrites. Successful reads are cached within each host process and invalidated through a non-secret local revision marker when another host changes the entry, avoiding repeated OS-store access for unchanged state without placing account data or tokens outside Keychain.

The adapter lazily loads `@napi-rs/keyring` and never falls back to plaintext when the OS store is locked, inaccessible, or unsupported. `createKeychainMcpCredentialRepository(...)` builds on it to implement the portable MCP credential and pending-authorization repositories as one versioned Keychain document. It preserves principal/server scoping, compare-and-swap revisions, and single-use state-bound callback transactions across local processes. The embedding product owns OAuth policy, target-specific native packaging, and which platforms opt into shared storage. A Keychain entry is not authorization for a model call by itself.
...
`createNodeHostMcpOAuthProvider(...)` composes that callback validation with the SDK OAuth protocol and a host-owned credential repository. The embedding host supplies the reviewed redirect URL and an authorization callback that opens the browser and returns the received callback URL; Node-host handles PKCE, dynamic client registration, token exchange, persistence, and refresh. `resolveOAuthProvider` threads that provider through remote tool/resource/prompt connections, while `authorizeOAuthNetwork` separately gates OAuth discovery/token destinations. Every mediated fetch rejects redirects. The remote adapter retries a connection at most once after authorization completes.
...
`createNodeHostMcpRemoteResourcePrompts(...)` and `createNodeHostMcpStdioResourcePrompts(...)` expose the same host-resolved servers as a turn-bound resource/prompt provider. Catalogs are paginated and bounded; unavailable servers are isolated. Reads and prompt expansion open a fresh connection, re-run the remote destination or local launch authorization, enforce the discovery principal/session/turn, forward cancellation with a fixed timeout, bound model-visible output, and close the client afterward. The provider does not authorize resources or prompts from a server omitted by the host, add persistent connections, discover config, or decide UI policy.

Remote and stdio options may supply `onElicitation` for host-owned form interaction. Node-host advertises form elicitation only when that callback exists, normalizes and bounds the server schema, validates/coerces accepted values, binds the request to the current principal/session/turn, and cancels promptly with the turn. URL-mode elicitation is always declined in this adapter; it is distinct from the explicit OAuth provider/browser callback flow.
