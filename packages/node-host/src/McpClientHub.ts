...
import type {
  CoreModelJsonSchema as JsonSchema,
  CoreModelToolDefinition as ToolDefinition,
...
export interface McpHubRequestContext {
  readonly principal: import("@agentlink/core").AgentPrincipal;
  readonly sessionId: string;
  readonly turnId: string;
}

export interface McpToolAuthorizationRequest {
  readonly context?: Readonly<McpHubRequestContext>;
...
/** Credential storage and browser OAuth belong to the consuming host. */
export interface McpHubOAuthProvider extends OAuthClientProvider {
  ...
  invalidateCredentials(kind: "client" | "tokens" | "all"): Promise<void>;
  debugStateSnapshot(label: string): Promise<void>;
}
...
export type McpNativeConnectionRequest =
  | {
      readonly context?: Readonly<McpHubRequestContext>;
      readonly config: Readonly<McpServerConfig>;
  ...
  | {
      readonly config: Readonly<McpServerConfig>;
      readonly context?: Readonly<McpHubRequestContext>;
      readonly transport: "sse" | "streamable-http" | "http";
      readonly url: string;
      readonly signal: AbortSignal;
    };

/** Host capabilities are explicit; plugin transports must never silently use native defaults. */
export interface McpHubHost {
  /** Immutable authenticated turn identity for a request-scoped hub. */
  readonly requestContext?: Readonly<McpHubRequestContext>;
  /** Current operation authority for a session-owned hub; undefined when idle. */
  readonly getRequestContext?: () => Readonly<McpHubRequestContext> | undefined;
  /** Called for every native connection/reconnect before OAuth startup or transport launch. */
  readonly authorizeNativeConnection: (
    request: McpNativeConnectionRequest,
  ) => boolean | Promise<boolean>;
  readonly createOAuthProvider?: (
    serverName: string,
    serverUrl: string,
  ) => McpHubOAuthProvider | Promise<McpHubOAuthProvider>;
  readonly notify: (
    level: "warning" | "error",
    message: string,
    action?: string,
  ...
  readonly buildPluginEnvironment: (
    base: Record<string, string>,
    configured: Record<string, string> | undefined,
    boundary: { pluginRoot: string; pluginData: string },
  ) => Record<string, string>;
  readonly fetch: typeof globalThis.fetch;
  /** Wrap native HTTP fetch per request, including SDK discovery and redirects. */
  readonly createNativeFetch: (
    config: Readonly<McpServerConfig>,
    baseFetch: typeof globalThis.fetch,
    context?: Readonly<McpHubRequestContext>,
  ) => typeof globalThis.fetch;
  readonly createPluginFetch: (
    url: string,
    headers: Readonly<Record<string, string>> | undefined,
    baseFetch: typeof globalThis.fetch,
  ) => typeof globalThis.fetch;
  readonly createPluginSseTransport: (
    url: URL,
    options: {
  ...
  readonly createSchemaValidator: () => Promise<McpSchemaValidatorProvider>;
}
...
export interface McpSchemaValidatorProvider {
  getValidator<T>(
    schema: object,
  ): (
    input: unknown,
...
export class McpClientHub {
  ...
  private showReauthenticateNotification(
    ...
    void this.host
      .notify("error", `AgentLink: ${message}`, action)
      .then((selection) => {
  ...
  private async getSchemaValidator(): Promise<McpSchemaValidatorProvider> {
    if (!this.schemaValidator) {
      this.schemaValidator = await this.host.createSchemaValidator();
    }
  ...
  private async performConnectServer(
    ...
    // Plugin boundary failures remain inside the established connection error path.
    const environment =
      cfg.provenance?.kind === "agent-plugin"
        ? undefined
        : this.resolveStdioEnvironment(cfg);
    if (!(await this.authorizeConnection(cfg, signal, environment))) {
      signal.throwIfAborted();
      throw new Error(
        `MCP server '${cfg.name}' connection denied by host policy`,
      );
    }
    signal.throwIfAborted();

    // Get or create OAuth provider for HTTP servers
    let oauthProvider: McpHubOAuthProvider | undefined;
    const isHttpServer =
      cfg.type === "sse" ||
      cfg.type === "streamable-http" ||
      cfg.type === "http";
    if (isHttpServer && cfg.url && this.host.createOAuthProvider) {
      ...
      if (!oauthProvider) {
        oauthProvider = await this.host.createOAuthProvider(cfg.name, cfg.url);
        signal.throwIfAborted();
    ...
    try {
      ...
      // A rejected dynamic client registration will fail identically on every
      // retry until the server or client configuration changes.
      const registrationRejection = this.oauthRegistrationRejectionCode(err);
      if (
        registrationRejection &&
        !this.isRedirectMismatchMessage(normalizedErr)
      ) {
        const message = `OAuth client registration was rejected (${registrationRejection}) for '${cfg.name}'. Automatic retries are paused. Use Reconnect or Reauthenticate to try again.`;
        entry.status = "error";
        entry.error = message;
        this.authFailureCounts.delete(cfg.name);
  ...
  private authErrorKind(
    err: unknown,
  ): Extract<McpAuthEvent, { type: "connect_auth_failure" }>["errorKind"] {
  ...
  private oauthRegistrationRejectionCode(err: unknown): string | undefined {
    if (!(err instanceof Error)) return undefined;
    // SDK OAuth errors expose errorCode; avoid instanceof across CJS/ESM copies.
    const code = (err as { errorCode?: unknown }).errorCode;
    if (
      code === "invalid_client_metadata" ||
      err.name === "InvalidClientMetadataError"
    )
  ...
  private async handleRuntimeAuthFailure(
    ...
    this.authFailureCounts.delete(serverName);
    this.onStatusChange?.(this.getServerInfos());
    void this.host.notify("warning", `AgentLink: ${message}`);
    this.log(`[mcp:${serverName}] ${message}`);
    this.triggerRuntimeReconnect(entry.config);
    return message;
  }
  ...
  private resolveStdioEnvironment(
    cfg: Readonly<McpServerConfig>,
  ): Record<string, string> | undefined {
    if ((cfg.type ?? "stdio") !== "stdio") return undefined;
    const baseEnvironment = this.host.baseEnvironment();
    if (cfg.provenance?.kind !== "agent-plugin") {
      return { ...baseEnvironment, ...cfg.env };
    }
    if (!cfg.pluginRoot || !cfg.pluginData) {
      throw new Error(
        `Plugin MCP server '${cfg.name}' is missing its authorized root/data boundary.`,
      );
    }
    return this.host.buildPluginEnvironment(baseEnvironment, cfg.env, {
  ...
  private async authorizeConnection(
    ...
    if (cfg.provenance?.kind === "agent-plugin") return true;
    if ((cfg.type ?? "stdio") === "stdio") {
      if (!cfg.command)
        throw new Error(`Server '${cfg.name}' is stdio but missing 'command'`);
      return this.host.authorizeNativeConnection({
        context: this.host.getRequestContext?.() ?? this.host.requestContext,
        config: cfg,
        transport: "stdio",
        command: cfg.command,
        args: cfg.args ?? [],
        cwd: cfg.cwd,
        env: environment ?? {},
        remoteServerUrl: parseMcpRemoteInvocation(cfg)?.serverUrl,
        signal,
      });
    }
    if (!cfg.url)
      throw new Error(`Server '${cfg.name}' is ${cfg.type} but missing 'url'`);
    return this.host.authorizeNativeConnection({
      context: this.host.getRequestContext?.() ?? this.host.requestContext,
      config: cfg,
      transport: cfg.type as "sse" | "streamable-http" | "http",
      url: cfg.url,
      signal,
    });
  }
  ...
  private createTransport(
    ...
    if (type === "sse") {
      ...
      if (isPlugin) {
        return this.host.createPluginSseTransport(new URL(cfg.url), {
          authProvider,
          mcpFetch: this.host.createPluginFetch(
            cfg.url,
            cfg.headers,
            connectionFetch,
          ),
      ...
      return new SSEClientTransport(new URL(cfg.url), {
        authProvider,
        fetch: this.host.createNativeFetch(
          cfg,
          connectionFetch,
          this.host.getRequestContext?.() ?? this.host.requestContext,
        ),
    ...
    if (type === "streamable-http" || type === "http") {
      ...
      return new StreamableHTTPClientTransport(new URL(cfg.url), {
        authProvider,
        fetch: isPlugin
          ? this.host.createPluginFetch(cfg.url, cfg.headers, connectionFetch)
          : this.host.createNativeFetch(
              cfg,
              connectionFetch,
              this.host.getRequestContext?.() ?? this.host.requestContext,
            ),
  ...
  async reauthenticateServer(name: string): Promise<void> {
    ...
    if (
      ...
      throw new Error(
        `MCP server '${cfg.name}' reauthentication denied by host connection policy`,
      );
    }
    ...
    if (isHttpServer && cfg.url && this.host.createOAuthProvider) {
      ...
      const provider = await this.host.createOAuthProvider(cfg.name, cfg.url);
  ...
  async callTool(
    ...
    if (
      (server.config.provenance?.kind === "agent-plugin" &&
        !this.options.isConfigCurrent) ||
      !(await this.configIsCurrent(server.config))
    ) {
      return {
        content: [
          {
    ...
    const authorization = this.options.onBeforeToolCall
      ? await this.options.onBeforeToolCall({
          context:
            options?.requestContext ??
            this.host.getRequestContext?.() ??
            this.host.requestContext,
          serverName,
    ...
    const configuredTimeout = server.config.timeout;
    const timeout =
