...
describe("MCP form elicitation protocol", () => {
  it("keeps request and response DTOs serializable", () => {
    expectTypeOf<McpFormElicitationRequest>().toMatchTypeOf<{
      id: string;
      serverName: string;
      message: string;
      fields: McpElicitationField[];
    }>();
    expectTypeOf<McpFormElicitationResponse>().toMatchTypeOf<
      | { id: string; action: "accept"; values: Record<string, unknown> }
