...
/** Adapts the CLI's repository-backed SDK provider to the shared hub lifecycle. */
export class CliMcpHubOAuthProvider implements McpHubOAuthProvider {
  ...
  async redirectToAuthorization(url: URL): Promise<void> {
    ...
    try {
      ...
      // The CLI SDK provider completes the code exchange internally, so its
  ...
  async debugStateSnapshot(_label: string) {
    // The CLI does not persist a debug snapshot of OAuth credentials.
  }
}
