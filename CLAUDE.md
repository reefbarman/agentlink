...
- Run `npm run telemetry:sessions` to inspect goal-level session outcomes: per-turn wall-clock decomposition (streaming vs tools vs blocked-on-background vs waiting-on-user), time-to-goal per task status, background agent cost/benefit, and named sanity indicators (blocked-wait ratio, spawn-before-first-action rate, empty-review rate). Use it before and after changing delegation/review prompts or approval mechanics to measure behavioral impact.
