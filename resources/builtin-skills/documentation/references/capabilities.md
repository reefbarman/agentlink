...
AgentLink is a VS Code extension whose built-in agent chats in stable independent tabs, edits files through diff views, runs commands in the integrated terminal, and uses VS Code diagnostics, symbol navigation, code actions, and rename support. Chat tabs can move between the sidebar and restorable editor panels; at least one tab remains docked, closing a popped panel docks it again, and switching presentation does not interrupt another tab's work. AgentLink also connects to MCP servers, spawns background agents, and can be remote-controlled from a browser.
