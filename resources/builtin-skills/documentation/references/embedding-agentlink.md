...
| `@agentlink/node-host`      | Optional Node implementations and composition helpers for explicit local capabilities, Keychain storage, and the shared Codex OAuth account pool.                                                                                                                              | Implicit authority, a terminal UI, a sandbox, a desktop shell, or a cloud deployment.                 |
| `@agentlink/workspace-host` | Private standalone local-project composition for durable chat, canonical project identity, providers, request bounds, scoped file tools, reviewed writes and commands, MCP, background writers, optional managed TypeScript/JavaScript intelligence, and CLI writer ownership. | Terminal rendering, sandbox confinement, interactive PTYs, broader platforms, or public distribution. |
...
| Standalone CLI                        | **Private MVP is runnable on macOS Apple Silicon.** It provides terminal chat, provider setup, durable project sessions, scoped file inspection, reviewed baseline-hash writes and patches, reviewed non-PTY commands, approved MCP, scoped background writers, optional managed TypeScript/JavaScript intelligence, writer ownership, recovery, cancellation, and request bounds. | Broader platforms, sandbox confinement, interactive or persistent PTYs, and public distribution.                                                                              |
...
Use the same engine and Node-host composition behind a CLI command loop. The private standalone CLI now supplies a stable local principal/data realm, session selection/resume policy, credential setup, bounded stores/recovery, event rendering, project-relative scoped file tools, a durable terminal approval flow, exact reviewed non-PTY commands with retained process output, approved stdio and HTTPS MCP, up to two scoped background writers, optional managed TypeScript/JavaScript intelligence, and cross-process CLI writer ownership. Its writes are pinned to saved-file SHA-256 baselines and verified after commit. Unsaved editor buffers remain invisible. Commands and child processes remain host-managed and unsandboxed; interactive or persistent PTYs and sandbox integration remain later host work.
