...
`agentlink.memory.mode = autonomous` enables typed low-authority memory. `/memory` opens a local manager without a model request: inspect/filter records and provenance, revisions, and audit history; forget/restore, undo, clear a confirmed scope, or import/export versioned archives. VS Code supports global and current-project scopes; projectless Browser Ask Agent supports global scope only. Both surfaces use the same helper-owned store under `~/.agentlink/`, and Browser Ask Agent does not require a connected VS Code window for memory. `/remember` is the model-assisted workflow for identifying durable candidates.

In either manager, use **Add memory** to save a fact or preference directly without a model request. Select a record and choose **Edit** to change its statement or kind. Save waits for the store's response; rejected saves keep the draft visible. If another session changed the record, cancel editing and select it again to load the latest revision before applying your changes. Global memories are shared across projects and Browser Ask Agent; project memories stay in the current VS Code project.

Search accepts Enter, and **Reset filters** clears the search, kind, and status filters. The result count distinguishes loaded records from total matches (the list is bounded to 100). Expand **Sources** for the supporting evidence and **Revisions** for previous statements and statuses. Record details also show confidence, last update, and expiry when present. Scope changes, other mutations, and closing the manager are disabled while editing; save or cancel the draft first. **Clear…** forgets the entire selected scope, including records hidden by filters, with undo available in recent activity.

Autonomous memory remains evidence only and uses `manage_memory` / `recall_memory`; it cannot authorize tools or override current user, repository, instruction, skill, or command evidence. Legacy global/project `.agentlink/memory.md` files and prior per-profile retrieval stores are imported idempotently and preserved for rollback rather than injected directly into the prompt.
