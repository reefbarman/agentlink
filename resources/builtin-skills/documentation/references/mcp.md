...
- AgentLink continues to use a configured `mcp-remote` command as the MCP transport. A proxy with cached credentials may start in the background so it can refresh silently. A cache-cold proxy waits until the next active agent turn before starting its first interactive login. If that initial startup fails, AgentLink pauses automatic startup retries instead of repeatedly relaunching the proxy and browser; use **Reconnect** or **Reauthenticate** to try again. A proxy that connected successfully and later drops still reconnects in the background.
- In VS Code-backed MCP connections, disabling or disconnecting a server cancels queued connection attempts and pending OAuth prompts/callbacks, including manual reauthentication. Cancelled work cannot later reopen sign-in or reconnect the server. An unanswered refresh-failure reauthentication prompt expires after 60 seconds; the browser callback retains its five-minute deadline. Use **Reauthenticate** to start a fresh attempt. Already-open browser tabs or VS Code notifications may remain visible, but late responses do not resume the cancelled flow.
...
- Large catalogs are progressively disclosed (`find_mcp_tools` / `call_mcp_tool`); the same approval model applies to MCP tools as to native ones. MCP runtime names containing characters that provider function APIs reject, or exceeding their portable length limit, are also routed through this bridge, so one incompatible server or tool name cannot invalidate the whole model request.
...
- Browser surfaces backed by VS Code: main-profile browser config is read-only; loopback Browser Ask Agent may configure local-process servers; LAN sessions may configure only secret-free HTTP/SSE servers.
- Standalone Desktop executes tools pre-authorized by `toolPolicy: "allow"` or `allowedTools`; tools using the default `ask` policy display an MCP approval card with allow-once and current-conversation tool/server choices. These conversation grants are runtime-only and disappear when the helper stops or the conversation is deleted. Standalone Desktop also lists and reads configured MCP resources and prompt templates without VS Code. MCP form elicitation uses the same validated activity-shelf controls as the remote browser and cancels with the active turn.
- The CLI uses the shared per-session MCP runtime to expose `list_mcp_resources`, `read_mcp_resource`, `list_mcp_prompts`, and `get_mcp_prompt` alongside connected server tools. Resource and prompt reads require a current enabled server definition, but do not prompt for per-call approval. Server form elicitation opens an interactive terminal control, validates submitted fields, and cancels when the operation or turn ends. Multiple calls from the same turn to one server share an elicitation owner. Hub connection status and warnings appear in the CLI status line; `agentlink mcp status` still reports configuration, not live connections. Use `agentlink mcp reauthenticate <server-name> --project <path>` from an interactive terminal to replace the CLI's OAuth credentials for an enabled remote server. It requires typed confirmation and a separate browser-opening confirmation after the OAuth destination passes the public HTTPS check. Existing tokens remain available if the new sign-in is cancelled or fails; successful sign-in replaces them.
- On macOS, a standalone remote MCP server that requires OAuth first shows an Ask Agent approval card naming the authorization origin. Only **Open browser** launches the system browser. OAuth browser, discovery, and token destinations must resolve entirely to public HTTPS addresses; private, loopback, link-local, reserved, credential-bearing, failed-DNS, and redirected destinations fail closed. The helper receives a state-bound callback on its loopback listener and stores dynamic client registration plus tokens in Keychain without exposing credentials to the Electron renderer or browser client. Silent refresh uses the same separately authorized OAuth destinations as sign-in, while ordinary MCP requests remain restricted to the configured HTTPS origin. A blocked OAuth network request fails rather than reopening browser sign-in. The desktop browser callback uses AgentLink's branded page and says **Authorization received / Finishing setup** until the app completes token exchange; it is not itself confirmation that the connection is ready. OAuth approval cards label the action **Open browser** and omit unavailable persistent-rule controls. The helper retains shared MCP connections between turns of the same session and retires them when effective configuration changes or the helper stops. URL-mode elicitation remains declined because it is separate from MCP transport OAuth. The standalone MCP Manager remains read-only for configuration, but its **Reauthenticate** action starts a fresh Desktop OAuth sign-in for an enabled remote server after an approval card and opens the system browser only after authorization approval. Existing tokens remain available if the new sign-in is cancelled or fails; successful sign-in replaces them. This action is unavailable during an active turn. Non-macOS shared credential adapters remain later parity work. CLI and standalone Desktop use distinct Keychain accounts for MCP OAuth; VS Code uses extension-host credential storage. Existing credentials in the former shared Keychain account are not migrated or deleted, so a new CLI or Desktop login may be needed. Reauthenticating one surface does not change the others.
