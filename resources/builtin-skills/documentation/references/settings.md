...
- `background.modelTiers` — machine-scoped named groups of registered local model IDs, each split into `cheap`, `balanced`, and `deep_reasoning` arrays. Groups can include Codex, OpenAI-compatible, or other configured models. Native background work defaults one tier below the foreground model; if no eligible model exists at that tier, routing fails instead of silently spending a higher tier. Explicit `model`, `modelTier`, and configured review targets remain authoritative
