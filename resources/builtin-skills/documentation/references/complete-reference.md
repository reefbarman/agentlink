...
- **Slash commands** — built-ins include `/new`, `/mode`, `/model`, `/condense`, `/context-doctor`, `/environment`, `/workspace`, `/checkpoint`, `/revert`, `/help`, `/fleet`, `/remember`, `/memory`, `/skills`, `/mcp`, `/mcp-config`, `/mcp-refresh`, `/btw`, `/worktree`, `/pair`, and `/usage`. `/usage` reads Codex subscription limits and token activity directly with the active AgentLink ChatGPT/Codex OAuth account; it does not require or invoke a locally installed Codex CLI. `/environment` opens the current workspace session's runtime environment, loaded instructions, and system prompt in the Chat Activity Shelf without a model request; `/workspace` reports the active AgentLink history directory and identity for diagnostics/recovery. Both are available in VS Code workspace sessions; `/workspace` remains VS Code-only because history migration has no browser write path. Custom commands and detected skills appear in the same picker.
