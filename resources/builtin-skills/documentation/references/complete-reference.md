...
For review delegations, use `reviewScope` to describe the target. AgentLink captures it into an immutable snapshot when the agent is spawned, so queued reviews are not affected by later edits or commits. Relative paths resolve from the executing project, while absolute paths inside any open workspace root are accepted. `working_tree` captures unstaged tracked changes and untracked files by default, with optional state and path filters; `commit_range` resolves a Git range immediately. These Git scopes must stay within one workspace/Git root; in multi-root workspaces, set `root` (absolute path or folder name) on `working_tree`/`commit_range` to pick which root is captured. `files` captures exact current file contents, can span roots, and is the appropriate choice for non-Git workspaces. `diff` accepts content already captured by the caller. Git scopes omit binary patch payloads but retain bounded binary-change metadata; `excludePaths` is pushed into Git before stdout buffering and checked again after capture, so excluded large diffs cannot exhaust the buffer first. Exact files above the 1 MB per-file limit are recorded as metadata entries with content omitted instead of failing the capture, and when the total capture exceeds the limit the error names the largest captured items so the scope can be narrowed without trial and error. Outside-root errors identify allowed roots and show an accepted example. The `review_findings` envelope includes `reviewedScope` (what was actually reviewed) and `emptyDiff` (true when the requested scope was empty or missing). Envelope extraction is tolerant: fenced JSON (including `jsonc`-style info strings, one-line fences, an unclosed final fence, or a repeated envelope where the last occurrence wins) and bare JSON in plain text are accepted, and tolerable finding deviations are normalized instead of rejected — synonym severities (for example `warning`, `nit`, `blocker`) map onto the canonical scale, absolute paths under a workspace root become root-relative, and invalid `line` values are dropped from the finding. Runs that still end without a valid envelope report `expectedResultIssue` (which candidate/field failed) alongside the preserved raw output in `partialOutput`.
...
| `reviewScope`     | object?      | Runtime-captured review target (`working_tree`, `files`, `commit_range`, or `diff`); supports `excludePaths`, and `root` for multi-root workspaces       |
...
Review and research task classes always receive soft budgets based on committed tool calls, successful model API turns, and elapsed time: cheap tasks get 24 calls / 10 turns / 6 minutes, balanced tasks get 48 / 16 / 10 minutes, and deep-reasoning tasks get 72 / 24 / 15 minutes. Reviews infer balanced or deep-reasoning tier from the request unless `modelTier` is explicit; research defaults to balanced unless `modelTier` is explicit. A review caller can override those work-unit limits, but `maxTokens`, `maxEstimatedCostUsd`, and `estimatedCostPerMillionTokens` are ignored for reviews so a large captured diff cannot consume the budget before surrounding-code inspection. Research callers can override any supported cap. Writable build, debug, design, verification, and general task classes ignore supplied budgets and run uncapped so implementation is not stopped mid-change.
