...
- Provider IDs are the values shown as `resolvedProvider` in background routing, such as `codex`, `anthropic`, or `openai-compatible:<connection-id>`.
...
- A model target is validated before the agent launches. Unregistered, foreground-only, chat-only (no tool use), unauthenticated, or cooling-down targets fail the spawn with a message naming the setting rather than silently falling back. Retired model IDs follow the same migration path as other model settings.
