...
- **`spawn_background_agent`** — Spawn a parallel background agent for independent work such as research while coding, writing/proposing tests while production code is implemented, non-conflicting code/docs/test slices, alternate debug hypotheses, tangential checks, or quick/thorough reviews. It returns immediately with a `sessionId`; continue useful foreground work instead of waiting right away. For writable background work, include explicit owned files/directories, files to avoid, allowed commands/tests, and conflict instructions. Use `taskClass: "readonly-research"` for pure read-only lookup/exploration.
