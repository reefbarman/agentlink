...
- **`get_context`** — Build a compact context pack for an explicit file: metadata, git status, diagnostics summary, symbol outline, bounded numbered content, and working-set status. Use `dedupe_unchanged_content: true` to omit unchanged content for an exact range already returned in this session; use `refresh: true` to force content back in.
