...
**Exception — non-text files:** Use `read_file` for local image files too — it supports provider-safe image formats (`.png`, `.jpg`, `.jpeg`, `.gif`, `.webp`) and returns them as inspectable image content. It also supports PDF text extraction. The built-in `Read` tool is only still allowed for file types `read_file` does not handle yet: **Jupyter notebooks** (`.ipynb` — rendered with cells + outputs). A PreToolUse hook enforces this automatically.
...
- **DO NOT use `Read` to read files.** Use `read_file` for text files, local image files, and PDFs. **Exception:** built-in `Read` is allowed only for Jupyter notebooks (a file type `read_file` does not handle yet).
...
- **DO NOT use `Glob` to find files.** Use `list_files`. If expected files may be hidden by ignore rules, pass `include_ignored: true` with recursive or pattern listing; pair it with `pattern` when possible (e.g. `*.pdf`) to avoid noisy/truncated results.
...
- **Working directory**: The response includes a `cwd` field showing the terminal's actual working directory after the command finished. This reflects any `cd` commands — do NOT run `pwd`, just read `cwd` from the response. The `cwd` parameter controls terminal selection (only idle default terminals with matching tracked cwd are eligible for auto-reuse); it does not `cd` an already-running terminal. If no matching terminal exists, a new terminal may be created. If `cwd` is absent from the response, shell integration was unavailable and the directory is unknown.
...
- Use `background: true` for long-running processes (dev servers, watch modes). Returns immediately with `terminal_id`. Use `get_terminal_output` with that `terminal_id` to check on progress, read accumulated output, and see if the command has finished. Background terminals are never auto-reused — always use `terminal_name` or `terminal_id` to target them.
...
- **Inline temp files** — For ephemeral CLI inputs (for example `gh --body-file`, short throwaway scripts, `curl --data @file`), prefer `execute_command.files` instead of quoting rich multi-line text in the shell or creating a repo-local temp file. Reference each file path with `$AL_FILE(name)`, e.g. `command: "gh pr comment 42 --body-file $AL_FILE(body)", files: [{ name: "body", ext: "md", content: "..." }]`. These files are written under the OS temp dir, previewed in the approval card, substituted as quoted POSIX paths, and deleted after the command. Do **not** use this for workspace file edits; use `write_file`/`apply_diff`. Not supported with `background: true`.
...
Built-in AgentLink chat also has additional in-process orchestration tools such as `spawn_background_agent`, `get_background_status`, `get_background_result`, `kill_background_agent`, and `set_task_status`. `set_task_status` accepts `completeTodos: true` with `status: "completed"` to mark the current todo list complete without a separate final `todo_write` call. Background spawns may route to AgentLink's native runtime or a configured ACP stdio backend; use `provider: "acp:<id>"` only when the user/project has configured that ACP backend and the task is appropriate for its current read-only/review posture. Those are not external MCP-server tools; do not use or document them as available to Claude Code unless you are operating inside the built-in AgentLink chat runtime.

When the built-in AgentLink chat is connected to other MCP servers, those servers may request URL-mode elicitation for browser-based flows. AgentLink surfaces these as explicit user prompts in VS Code and the browser gateway; never assume the URL has opened automatically, and do not ask for write/exec browser capabilities. The user must explicitly choose whether to open the `http`/`https` URL.
