...
The **Approve for Me** button above the chat input enables a session-only command policy. Safe commands continue to follow the configured static tier. For eligible sensitive commands, AgentLink makes a fresh one-shot model call with no tools or conversation history. The reviewer can approve only recognized, workspace-local mutations, project toolchain commands, and a narrow set of recoverable Git mutations. Unknown executables, dangerous or ambiguous commands, external paths/effects, inline command files, environment overrides, and forced validator exceptions always use the normal human approval card. Reviewer errors, invalid output, timeouts, cancellation, or policy changes also fall back to the human card. The mode consumes model quota, is not persisted across extension-host restarts, creates no approval rules, and does not populate the recent-human-approval cache.
