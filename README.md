...
- **Review behavior**: review task classes (e.g. `review_code`, `review_plan`) prefer opposite-provider routing when available and normally select that provider's preferred non-cheap flagship model (currently Claude Fable 5 for Anthropic and GPT-5.6 Sol for Codex when advertised).
