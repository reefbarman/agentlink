...
- **Review behavior**: review task classes (e.g. `review_code`, `review_plan`) prefer opposite-provider routing when available and normally select that provider's preferred non-cheap flagship model (currently Claude Opus 4.8 for Anthropic and GPT-5.6 Sol for Codex when advertised).
