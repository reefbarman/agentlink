...
- **Slash commands** — built-ins include `/new`, `/mode`, `/model`, `/condense`, `/context-doctor`, `/environment`, `/checkpoint`, `/revert`, `/help`, `/fleet`, `/remember`, `/memory`, `/skills`, `/mcp`, `/mcp-config`, `/mcp-refresh`, `/btw`, `/worktree`, `/pair`, and `/usage`. `/environment` opens the current workspace session's runtime environment, loaded instructions, and system prompt in the Chat Activity Shelf without a model request; it is available in VS Code and browser workspace tabs. Custom commands and detected skills appear in the same picker.
